...
const documentService = require('../../shared/services/documentService');
...
const hospitalService = {
  ...
  async getRegistrationDocuments(hospitalId) {
    ...
    if (!isUuid) {
      const devHospitals = authService.getDevHospitals();
      const dev = devHospitals.find((h) => h.id === hospitalId);
      return (dev?.documents || []).map((d) => ({
        id: d.id,
        hospitalId,
        documentType: d.documentType || d.document_type,
        documentNumber: d.documentNumber || d.document_number || '',
        issuingAuthority: d.issuingAuthority || d.issuing_authority || '',
        issueDate: d.issueDate || d.issue_date || '',
        expiryDate: d.expiryDate || d.expiry_date || '',
        customDocumentName: d.customDocumentName || d.custom_document_name || '',
        documentName: d.documentName || d.document_name || '',
        fileName: d.documentName || d.document_name || '',
        storagePath: d.storagePath || d.storage_path || '',
        documentStatus: d.documentStatus || d.document_status || 'verified',
  ...
  async getApprovedHospitals({ search = '', city = '', state = '', sortBy = 'name-asc', limit = 50, page = 1 } = {}) {
    ...
    const devHospitals = authService.getDevHospitals();
    let demoApproved = devHospitals.filter(
      (h) => ['APPROVED', 'approved', 'verified'].includes(h.status)
    );

    if (search) {
      const q = search.toLowerCase();
      demoApproved = demoApproved.filter(
    ...
    if (city && city !== 'all') {
      demoApproved = demoApproved.filter((h) => h.city && h.city.toLowerCase() === city.toLowerCase());
    }
    if (state && state !== 'all') {
      demoApproved = demoApproved.filter((h) => h.state && h.state.toLowerCase() === state.toLowerCase());
    }

    // Merge: Real Supabase approved hospitals + demo approved hospitals (avoiding duplicates, prefer real Supabase records)
    const combinedApproved = [...approvedList];
    const seenIds = new Set(approvedList.map((h) => String(h.id || '').toLowerCase()));
    const seenEmails = new Set(approvedList.map((h) => String(h.email || '').toLowerCase()));
    const seenRegNos = new Set(approvedList.map((h) => String(h.registration_no || h.registrationNo || '').toLowerCase()));

    demoApproved.forEach((demoHosp) => {
      ...
      if (!seenIds.has(demoId) && !seenEmails.has(demoEmail) && (!demoReg || !seenRegNos.has(demoReg))) {
        combinedApproved.push(demoHosp);
        seenIds.add(demoId);
